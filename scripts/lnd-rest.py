...
def sendPostRequest(endpoint,data=""):
	r = requests.post(base_url + endpoint, headers=headers, verify=cert_path, data=json.dumps(data))
	pprint(r.json())
	return r.json()

def sendGetRequest(endpoint, data=""):
	global base_url
	r = requests.get(base_url + endpoint.format(data), headers=headers, verify=cert_path)
	pprint(r.json())
